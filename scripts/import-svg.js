...
import * as util from '../src/common/ds-util/index.mjs';
...
function generateFile(type, iconMap) {
    for (const [name, themes] of iconMap) {
        ...
        for (const theme of THEME_NAMES) {
            ...
            const content = themes[theme]
                ? `export function symbolDS${shortTheme}() {
    ...
    return ${missingSVG.replace('{{name}}', name)};
        ...
        fs.writeFileSync(
            index,
            `import { symbolDS6 } from "./symbol.js";\nimport { symbolDS4 } from "./symbol[ds-4].js";\n<ebay-icon ...input _name="${name}" _type="${type}" _themes=[symbolDS6, symbolDS4]/>\n`
        );
    }
}
