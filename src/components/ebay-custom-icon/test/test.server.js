...
describe('test', () => {
    ...
    describe('transformer', () => {
        ...
        it('shows a warning message if the path format is not supported', () => {
            ...
            // this is the scenario for when one uses the path stored in a variable
        ...
        it('shows a warning message and does not set static var if path is not passed', () => {
            const mock = sinon.mock(console);
            mock.expects('warn').once().withArgs('The entered path format is not supported. Valid example: ' +
                '<ebay-icon type="inline" name="custom-add" path="./custom-symbols" />');
            const tagString = `<ebay-icon name="custom-add" type="inline" />`;
            runTransformer(transformer, tagString, componentPath);
            expect(context.getStaticVars()).not.to.have.key('icon_custom_add');
            mock.verify();
            mock.restore();
        });
