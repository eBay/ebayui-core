    ...
    describe(`given the tooltip with pointer ${pointer}`, () => {
        ...
        describe("when the host element is hovered", () => {
            ...
            it("then it sets the overlay styles correctly", () => {
                ...
                expect(overlayStyle).has.property(
                    "transform",
                    expectedStyles.overlayTransform
                );
                expect(overlayStyle).has.property(
                    "top",
                    expectedStyles.overlayTop
                );
                expect(overlayStyle).has.property(
                    "left",
                    expectedStyles.overlayLeft
                );
                expect(overlayStyle).has.property(
                    "right",
                    expectedStyles.overlayRight
                );
                expect(overlayStyle).has.property(
                    "bottom",
                    expectedStyles.overlayBottom
                );
            });
        });
    });
