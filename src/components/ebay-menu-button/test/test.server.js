...
describe('menu-button', () => {
    ...
    it('renders with reverse=true', async() => {
        ...
        expect(getByRole('menu')).with.class('menu-button__items--reverse');
    });

    it('renders with type=fake, reverse=true', async() => {
        const input = assign({ type: 'fake', reverse: true }, mock.Basic_2Items);
        const { getByText } = await render(template, input);
        expect(getByText(input.items[0].renderBody.text).closest('.fake-menu-button__items--reverse'))
            .does.not.equal(null);
    });
    ...
    it('renders with fix-width=true', async() => {
        ...
        const { getByRole } = await render(template, input);
        expect(getByRole('menu')).with.class('menu-button__items--fix-width');
    });

    it('renders with type=fake, fix-width=true', async() => {
        const input = assign({ type: 'fake', fixWidth: true }, mock.Basic_2Items);
        const { getByText } = await render(template, input);
        expect(getByText(input.items[0].renderBody.text).closest('.fake-menu-button__items--fix-width'))
            .does.not.equal(null);
    });
